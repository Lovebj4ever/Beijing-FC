        ...
        if(cursor.moveToFirst()){
            do{
                ...
                if((cursor.getString(cursor.getColumnIndex("type")).equals("倒数日")&&sumday!=0)||cursor.getString(cursor.getColumnIndex("type")).equals("纪念日")) {
                    ...
                    if(cursor.getString(cursor.getColumnIndex("type")).equals("纪念日")) {
                        dd = new Data(cursor.getString(cursor.getColumnIndex("name")) + " " + cursor.getString(cursor.getColumnIndex("year")) + "-" + cursor.getString(cursor.getColumnIndex("month")) + "-" + cursor.getString(cursor.getColumnIndex("day")), R.drawable.count);
                    }
                    else{
                        dd = new Data(cursor.getString(cursor.getColumnIndex("name")) + " " + cursor.getString(cursor.getColumnIndex("year")) + "-" + cursor.getString(cursor.getColumnIndex("month")) + "-" + cursor.getString(cursor.getColumnIndex("day")), R.drawable.rem);
                    }
                    new_data.add(dd);
                }
    ...
    public boolean onOptionsItemSelected(MenuItem item){
        switch(item.getItemId()){
            case R.id.calculator:
                ...
                break;
            case R.id.d:
                Intent intent2 = new Intent(SpecialDay.this,MainActivity.class);
...
public boolean onKeyDown(int keyCode, KeyEvent event){
    if(keyCode==KeyEvent.KEYCODE_BACK){
        Intent intent=new Intent(SpecialDay.this,MainActivity.class);
        startActivity(intent);
    }
    return true;
}
